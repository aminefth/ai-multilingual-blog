...
class AIService {
  ...
  async generateBlogContent(options) {
    ...
    const cacheKey = `ai:content:${Buffer.from(JSON.stringify({
      title, topic, keywords, tone, wordCount, language
    })).toString('base64')}`;
  ...
  async improveContent(content, improvements = [], language = 'en') {
    const improvementPrompts = {
      ...
      readability: 'Improve readability with shorter sentences, clearer language, and better headings',
  ...
  async translateContent(content, title, sourceLanguage = 'en', targetLanguage) {
    ...
    const languageContexts = {
      ...
      de: { name: 'German', context: 'German business culture focusing on precision, quality, and technical excellence' },
      es: { name: 'Spanish', context: 'Spanish/Latin American business culture with warm, relationship-focused approach' }
    };
  ...
  async generateTitles(topic, style = 'engaging', language = 'en') {
    ...
    try {
      ...
      const result = JSON.parse(completion.choices[0].message.content);
      return Array.isArray(result.titles) ? result.titles : [result.titles || `${style.charAt(0).toUpperCase() + style.slice(1)} Guide to ${topic}`];
    } catch (error) {
