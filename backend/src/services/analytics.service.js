...
class AnalyticsService {
  ...
  async getRevenueStats(period = '30d') {
    ...
    const [affiliateRevenue, subscriptionRevenue, totalUsers, newUsers] = await Promise.all([
  ...
  async getTopPosts(limit = 10) {
    ...
    const topPosts = posts.map((post) => {
      const totalClicks =
        post.affiliateLinks?.reduce((sum, link) => sum + (link.clicks || 0), 0) || 0;
      const totalRevenue =
        post.affiliateLinks?.reduce((sum, link) => sum + (link.revenue || 0), 0) || 0;
      const conversionRate =
        totalClicks > 0
          ? (
              ((post.affiliateLinks?.reduce((sum, link) => sum + (link.conversions || 0), 0) || 0) /
                totalClicks) *
              100
            ).toFixed(2)
          : 0;
  ...
  async getAffiliatePerformance() {
    ...
    const result = Object.values(performance)
      .map((item) => ({
        ...item,
        conversionRate: item.clicks > 0 ? ((item.conversions / item.clicks) * 100).toFixed(2) : 0,
      }))
  ...
  async getAffiliateRevenue(startDate) {
    ...
    return posts.reduce((total, post) => {
      const postRevenue = post.affiliateLinks.reduce((sum, link) => sum + (link.revenue || 0), 0);
