...
const getAffiliateTools = catchAsync(async (req, res) => {
  ...
  const tools = Object.values(toolsMap).map((tool) => ({
    ...tool,
    conversionRate:
      tool.totalClicks > 0 ? ((tool.totalConversions / tool.totalClicks) * 100).toFixed(2) : 0,
  }));
...
const getAffiliateReport = catchAsync(async (req, res) => {
  ...
  // Calculate totals
  const totalClicks = filteredPerformance.reduce((sum, item) => sum + item.clicks, 0);

  const totalConversions = filteredPerformance.reduce((sum, item) => sum + item.conversions, 0);

  const totalRevenue = filteredPerformance.reduce((sum, item) => sum + item.revenue, 0);
...
const addAffiliateLink = catchAsync(async (req, res) => {
  ...
  if (!link.text || !link.url || !link.tool) {
    throw new ApiError(httpStatus.BAD_REQUEST, 'Link must contain text, url, and tool');
  }
...
async function getTopPostsWithAffiliateLinks(tool) {
  ...
  return posts.map((post) => {
    ...
    const totalClicks = post.affiliateLinks.reduce((sum, link) => sum + (link.clicks || 0), 0);
    ...
    const totalRevenue = post.affiliateLinks.reduce((sum, link) => sum + (link.revenue || 0), 0);
    ...
    return {
      ...
      clickThroughRate: post.views > 0 ? ((totalClicks / post.views) * 100).toFixed(2) : 0,
